...
enum WordFontSlot: UInt8, CaseIterable {
    case ascii
    case hAnsi
    case eastAsia
...
struct WordRFonts: Equatable {
    var ascii: WordFontDecl?
    var hAnsi: WordFontDecl?
    var eastAsia: WordFontDecl?
    var cs: WordFontDecl?
    ...
    var hintsEastAsia: Bool { hint == "eastAsia" }
    ...
    var isEmpty: Bool { self == WordRFonts() }

    subscript(slot: WordFontSlot) -> WordFontDecl? {
        get {
            switch slot {
            case .ascii: return ascii
            case .hAnsi: return hAnsi
            case .eastAsia: return eastAsia
            case .cs: return cs
            }
        }
        set {
            ...
            case .hAnsi: hAnsi = newValue
            case .eastAsia: eastAsia = newValue
            case .cs: cs = newValue
            }
        }
    }
    ...
    func effectiveSlot(_ slot: WordFontSlot) -> WordFontSlot {
        guard slot == .eastAsia, case .literal("Times New Roman") = eastAsia, ascii == hAnsi else { return slot }
        return .ascii
    }
    ...
    func family(for slot: WordFontSlot, script: String?, theme: WordThemeFonts) -> String? {
        switch self[effectiveSlot(slot)] {
        case let .literal(name): return name
        case let .theme(ref): return theme.family(forThemeRef: ref, script: script)
        case nil: return nil
        }
    }
}
...
struct WordThemeFonts: Equatable {
    struct Scheme: Equatable {
        ...
        var byScript: [String: String] = [:]
    }

    var major = Scheme()
    var minor = Scheme()

    var isEmpty: Bool { self == WordThemeFonts() }
    ...
    private enum Which { case major, minor }
    private enum Role { case latin, eastAsian, complex }
    ...
    private static func target(of ref: String) -> (Which, Role)? {
        switch ref {
        case "majorAscii", "majorHAnsi": return (.major, .latin)
        case "majorEastAsia": return (.major, .eastAsian)
        case "majorBidi": return (.major, .complex)
        case "minorAscii", "minorHAnsi": return (.minor, .latin)
        case "minorEastAsia": return (.minor, .eastAsian)
        case "minorBidi": return (.minor, .complex)
        default: return nil
        }
    }
    ...
    static func scriptCode(for klass: ScriptClass) -> String? {
        switch klass {
        case .latin: return "Latn"
        case .hangul: return "Hang"
        case .kana: return "Jpan"
        case .han, .eastAsianOther, .complex, .other, .common, .inherited, .extend: return nil
        }
    }
}
...
/// one's family.
struct WordSlotKey: Equatable {
    let slot: WordFontSlot
    let script: String?
}
...
enum WordFontBlockTable {
    ...
    static func slot(for scalar: Unicode.Scalar, hintsEastAsia: Bool) -> WordSlotKey? {
        let klass = UnicodeScript.of(scalar)
        if klass.isAbsorbing { return nil }
        return WordSlotKey(slot: slot(forValue: scalar.value, hintsEastAsia: hintsEastAsia),
                           script: WordThemeFonts.scriptCode(for: klass))
    }
    ...
    static func slot(forValue value: UInt32, hintsEastAsia: Bool) -> WordFontSlot {
        if value > 0xFFFF { return .eastAsia }
        guard let row = row(containing: value) else { return .hAnsi }
        return hintsEastAsia ? row.hinted : row.plain
    }

    private struct Row {
        let start: UInt32
        let end: UInt32
        let plain: WordFontSlot
        let hinted: WordFontSlot
    }

    private static func row(containing value: UInt32) -> Row? {
        var low = 0
        var high = rows.count - 1
        while low <= high {
            let mid = (low + high) / 2
            let row = rows[mid]
            if value < row.start { high = mid - 1 } else if value > row.end { low = mid + 1 } else { return row }
        }
        return nil
    }
