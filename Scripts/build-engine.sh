...
#
# Deliberately NOT run by make-app.sh. The rhwp binary is committed because its source lives
# outside this repo; this engine's source is right here, so committing a build product of it would
# be a second copy that can disagree with the code beside it. The cost is that a build which wants
# the Rust engine has to ask for it — which is also what keeps the default build Rust-free while
# the engine is still being proven.
#
