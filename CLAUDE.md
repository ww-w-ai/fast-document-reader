...
text, tables with merged cells, cell-level images/lists and nested tables drawn as grids (invariant 168), images, links (including internal
...
- **`RenderTree` is the only office wire** (`EnvelopeV1`, `schema_version` 1), read from the SAME parse
  handle the reader already opened (`fastdoc_office_tree_json`; `fastdoc_read_office_tree` for a read with
  no handle). `Render/Office/RenderTreeWire.swift` mirrors the envelope and
  `Render/Office/RenderTreeOfficeAdapter.swift` turns it into `OfficeReadResult`/`OfficeBlock` — the HOST's
  vocabulary, not a second contract. schema-v5 JSON and its export layer were deleted on 2026-09-05; do not
  build a second projection "for compatibility" (invariants 171, 172).
...
- `hosts/avalonia/` — the Windows/Linux host of the same engine; what each of its files decides is `ARCHITECTURE.md`'s `hosts/` section.
...
`INVARIANTS.md` (repo root) is this app's manual: 206 numbered entries, each the measurements
...
| `Info.plist`, document types, default handler, sandbox, entitlements, the Gatekeeper document sheet | 6, 8, 9, 22, 28, 43, 69, 70, 156 |
| menus, Open Recent, titlebar accessory, sidebar, outline panel | 7, 21, 23, 26, 27, 148 |
| markdown parsing or a new markdown feature, the markdown wire | 12, 41, 101, 122, 123, 128, 130, 131, 162, 192, 193, 194 |
| keyboard navigation, reading cursor, margin numbers, jump | 14, 15, 71 |
| editing, save, text encoding, splice render | 16, 17, 18, 19, 20 |
| reflow, resize, `precomputeLayout`, first-paint cost | 24, 25, 32, 48, 49, 55, 56, 113, 117, 168 |
...
| office readers (docx / odt / hwp), dispatch, rhwp, headings | 29, 33, 44, 45, 73, 75, 78, 79, 81, 94, 110, 112, 115, 133, 153, 154, 155, 161, 163, 164, 165, 166, 169, 170, 171, 172, 189, 190, 195 |
| tests, corpus probes, "is this check actually reached" | 5, 29, 30, 34, 35, 41, 103, 104, 106, 109, 110, 111, 112, 113, 114, 116, 119, 122, 124, 134, 135, 136, 157, 171, 172, 198, 199, 200, 201, 202 |
| `RenderTheme` tokens, `OfficeTextBuilder`, style resolution | 36, 37, 97, 107, 163, 165 |
| tables — build, widths, borders, merges, attribute cost, nested grids, empty cells | 39, 42, 47, 50, 51, 72, 74, 76, 129, 144, 149, 152, 158, 161, 162, 163, 164, 166, 168, 169, 170, 195, 196, 197, 203 |
| fonts, per-script slots, substitution | 52, 53, 93, 95, 146, 163 |
| `swiftshim` strings, UTF-16 offsets, `SwiftString`, attributed-string length | 122, 123 |
| the engine's measurement port (`RustEngineMeasure`), terminator attributes | 51, 105, 161 |
| the comments panel | 38 |
| paged documents — zoom, page band, header/footer, page outline | 46, 57, 58, 60, 62, 77, 78, 118, 121, 147, 153, 161, 163, 189, 190, 203, 204, 205 |
| footnotes — the band fixpoint, where a note lands, the settle's round budget | 98, 99, 161 |
| printing, `--pdf`, `--extract`, anything headless | 40, 59, 66, 70, 102, 116, 120, 125, 126, 127, 132, 151, 164, 167, 171, 187, 188, 204 |
| a table that crosses a page boundary | 61, 64, 72, 96, 142, 145, 204 |
| multi-column layout, `ColumnGeometry`, column flow | 100, 108, 143, 144, 145, 150, 161 |
...
| the Avalonia host (Windows/Linux) — threads, PDF, publish, the win-arm64 cross-build | 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194, 195, 196, 197, 198, 199, 200, 201, 202, 203, 204, 205, 206 |
...
- **A DOWNLOADED document opened with FastDoc shows Gatekeeper's "Apple could not verify … is free of malware" naming the DOCUMENT → it is macOS, not the app, and no Info.plist change fixes it.** Apple DTS (developer.apple.com/forums/thread/795994): a quarantined document whose handler was set by "Open With" / default-app carries `LSRiskCategoryHasRedirectedBinding` into the assessment, Gatekeeper turns "paranoid", and the sheet fires; a SANDBOXED handler (the App Store build) can get a "damaged" message instead. Filed as FB19468486 / FB19623735; DTS: "no developer-side code fix is available". The user's way out is `xattr -d com.apple.quarantine <file>` (or opening from inside the app). Do not chase UTI/role/notarization — all three were ruled out. Separately, a dev machine also has every dev bundle ever built registered with LaunchServices, all named FastDocReader in Open With; `lsregister -dump | grep 'path:.*FastDocReader.app'` lists them and `lsregister -u <path>` drops them.
- **The space-bar preview shows no ⚡️FastDoc badge, or an old build → a STALE dev extension is drawing it.** Every dev bundle ever built — worktrees, a scratchpad `T.app`, the repo's own `FastDocReader.app` — registers its `ai.ww-w.fast-md-reader.dev.quicklook` appex with LaunchServices, and macOS may hand the preview to any of them (or to a competitor's) long after the bundle is gone. Diagnose with `pluginkit -m -v -p com.apple.quicklook.preview` and `ps` for which `.appex` process actually ran; fix by `pluginkit -r <every dev path>` until only `/Applications/…/QuickLookPreview.appex` is listed, then `killall quicklookd QuickLookUIService`. With the field clean, our extension wins `.md` and `.hwp` even with MDViewer's enabled (re-measured 2026-09-04); `.docx`/`.odt` stay with Apple's built-in (invariant 68), so no docx fix is visible there.
