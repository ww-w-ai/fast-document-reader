...
| 0038 | `cellDiagonal` — the resolved ANSWER to "is this a drawn cell diagonal, and which way", from the parser's own `BorderFill::cell_diagonal`, which its table editor now calls too. HWP splits the fact across a type field, two `attr` nibbles and a centre-line bit; two thirds of the raw declarations in 637 documents carry a type but no direction, so a host judging for itself rules lines across cells the document left plain. 47 documents / 208 cells actually draw one |
...
| 0040 | Footnote bodies leave the body flow for a document-level `footnotes[]` (shaped like the existing header/footer entries), and a note's reference marker carries `noteRef` + `noteRefKind` so a host can tell a marker from an exponent and match it to its note. ENDNOTES are untouched: measured over 637 samples, 1,832 endnotes across 33 documents already sit where an endnote belongs, against 333 footnotes across 22 documents that do not |
| 0043 | Each column's own WIDTH, not just how many there are — the renderer already resolves a table's grid, and without it a host re-derives a placeholder for every column the document sized by merge instead of by number |
| 0048 | The column-width resolver applied to the EXPORT, so a merged cell's column no longer arrives as HWP's 1800-unit placeholder. Measured on the 편람: 336 placeholder columns → 0, and the 기안문 table's width matched rhwp's own render exactly |
| 0049 | `heightPt` — a cell's OWN declared height, so a host can tell a decorative band the document sized (213 rows under 5pt, 198 of them empty) from a row sized by its text; plus PUA display characters expanded in `build_span`, so a symbol the document draws through a private code point (13 distinct, 97 uses in the 편람) arrives as the character rather than as U+F0848 |
